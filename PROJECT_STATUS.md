...
| _(pending)_ | (hot-drinks-5) | Hot drinks expanded to coffee/decaf/tea/herbal/peppermint (shared HOT_DRINKS list); summary breakdown now uses words, not icons. **Needs Run Migrations** (decaf_count/herbal_tea_count/peppermint_tea_count) | 🕐 Not yet tested |
...
### Notifications: admin indicator + bell scope (2026-07-28)
- [x] **Admin can see who has notifications on** — `GET /api/family/members` now attaches a `hasPush` boolean per member **only when the caller is an admin** (from `SELECT DISTINCT user_id FROM push_subscriptions`, guarded), keeping it private to admins. In **Manage mode** on the Family tab each member card shows 🔔 (has ≥1 push subscription) or a muted 🔕 (none). No schema change.
- [x] **Bell only rings people at the château** — the meal/apéritif bell (`POST /api/push/bell`) now calls new **`sendPushToPresent`** (`api/push/_send.ts`) instead of `sendPushToAll`: it joins `visits` and targets only approved subscribers whose `coming` visit includes today (`arrive_date <= CURRENT_DATE <= depart_date`). So family at home no longer get "🍽 Dinner is ready!". (The community "new family member" push still goes to everyone.) Inclusive of arrival/departure days; ignores per-meal skips for now.
...
### Meals print: lunch/dinner on separate pages (2026-07-27)
- [x] Each meal grid in `printDinnerGrid` is now wrapped in a `<section class="meal">`; the Dinner section gets a `break` modifier with `.meal.break{break-before:page;page-break-before:always}` so **Lunch prints on page 1 and Dinner starts on page 2**. CSS/markup only.

### Apéritif print layout (2026-07-27)
- [x] **Compact content-width cards** — the apéritif printout (`printAperitifs`) used `column-width:58mm` columns with `.dl{flex:1}`, which stretched the drink name so the ×count was shoved to the far edge of a wide column. Switched to a **flex-wrap of content-width cards** (`display:flex;flex-wrap:wrap`) and removed `flex:1`, so each day's card is only as wide as its longest drink name + count, the number sits right after the name, and the little cards flow/wrap across the page. CSS-only; no data/HTML change.
